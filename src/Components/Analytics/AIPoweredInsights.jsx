...
const AIPoweredInsights = () => {
  return (
    ...
    >
        <h2 className='text-lg font-medium mb-4 text-gray-400'>AI-powered Insights</h2>
        <div className="space-y-4">
            {
                INSIGHTS.map((item,index)=>(
                    <div key={index} className="flex items-center space-x-3">
                        <div className={`p-2 rounded-full ${item.color} bg-opacity-50`}>
                            <item.icon className={`h-6 w-6 ${item.color}`}/>
                        </div>
                        <div className="text-gray-100">{item.insight}</div>
                    </div>
                ))
            }
        </div>
